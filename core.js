...
function makeCore(data){
 ...
 function bank(u){const qs=u.examples.map((e,i)=>({...e,id:u.id+'-g'+i,unit:u.id,kind:i%2?'input':'choice',word:null,source:'grammar'}));u.focus.forEach((id,i)=>{const w=words[id],s=w.senses.filter(s=>levels.indexOf(s.level)<=levels.indexOf(u.level)&&s.example&&s.translation).at(-1);if(!s)return;const others=[...new Set(data.words.filter(x=>x.id!==id&&x.pos===w.pos).map(x=>x.senses[0].zh).filter(x=>x!==s.zh))],choices=others.slice(i,i+3).length===3?others.slice(i,i+3):others.slice(0,3);if(choices.length<3)return;qs.push({id:u.id+'-v'+i,unit:u.id,kind:'choice',prompt:s.example+'\n句中「'+w.lemma+'」的含义是？',answer:s.zh,choices,translation:s.translation,explanation:s.pattern||'结合词条义项与语境判断。',word:id,source:'vocabulary'});});return qs;}
