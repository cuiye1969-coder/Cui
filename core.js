...
function makeCore(data){
 ...
 function bank(u){const qs=u.examples.map((e,i)=>({...e,id:u.id+'-g'+i,unit:u.id,kind:i%2?'input':'choice',word:u.focus[i%u.focus.length],source:'grammar'}));u.focus.forEach((id,i)=>{const w=words[id],s=w.senses.filter(s=>levels.indexOf(s.level)<=levels.indexOf(u.level)).at(-1)||w.senses[0],others=data.words.filter(x=>x.id!==id&&x.pos===w.pos).map(x=>x.senses[0].zh).filter(x=>x!==s.zh);qs.push({id:u.id+'-v'+i,unit:u.id,kind:'choice',prompt:s.example+'\n句中「'+w.lemma+'」的含义是？',answer:s.zh,choices:[...new Set(others)].slice(i,i+3).length===3?[...new Set(others)].slice(i,i+3):[...new Set(others)].slice(0,3),translation:s.translation,explanation:s.pattern||'结合例句理解这个义项。',word:id,source:'vocabulary'});});return qs;}
